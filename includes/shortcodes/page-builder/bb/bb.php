			...
			//Preparing output
				if ( 'all' === $shortcode ) {
					...
					$output = wp_parse_args( wma_shortcodes()->get_definitions()['bb_plugin'][ $shortcode ], array(
							...
							'dir'             => trailingslashit( WMAMP_INCLUDES_DIR ) . 'shortcodes/page-builder/bb/modules/' . $shortcode . '/',
							'url'             => trailingslashit( WMAMP_INCLUDES_URL ) . 'shortcodes/page-builder/bb/modules/' . $shortcode . '/',
							'output'          => '',
							'output_children' => '',
							'params'          => array(),
			...
			//Include files
				if ( ! empty( $defs ) ) {
					foreach ( $defs as $module => $def ) {
						$module_file_path = trailingslashit( WMAMP_INCLUDES_DIR ) .'shortcodes/page-builder/bb/modules/' . $module . '/' . $module . '.php';
